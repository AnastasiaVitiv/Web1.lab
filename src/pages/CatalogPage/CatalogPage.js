...
import { addToCartForUser } from "../../utils/cartUtils";
...
function CatalogPage({ searchTerm = "" }) {
    ...
    const handleAddToCart = async (ship, type) => {
        const currentEmail = localStorage.getItem('currentUserEmail');
        
        if (!currentEmail) {
            alert('Будь ласка, увійдіть до системи для додавання товарів до кошика');
            return;
        }
        ...
        const productData = {
            ...
            selectedType: type,
            quantity: 1,
            price_per_ton: ship.price_per_ton,
            price_per_person: ship.price_per_person,
            tonnage: ship.tonnage,
            passengers: ship.passengers
        };
        
        try {
            logButtonClick('log/add-to-cart', {
                shipId: ship.id,
                shipName: ship.name,
                type: type,
            ...
            const success = addToCartForUser(currentEmail, productData);
            
            if (success) {
                alert(`${ship.name} додано до кошика!`);
                console.log(`Додано до кошика користувача ${currentEmail}:`, productData);
            } else {
